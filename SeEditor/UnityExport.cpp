...
bool WriteBinaryFile(std::filesystem::path const& path, std::span<const std::uint8_t> bytes)
{
    std::ofstream out(path, std::ios::binary | std::ios::trunc);
    if (!out)
        return false;
...
bool EnsureUnitySifImportEditorScript(std::filesystem::path const& unityRoot, std::string& error)
{
    std::error_code ec;
    const std::filesystem::path editorDir = unityRoot / "Assets" / "Editor";
    std::filesystem::create_directories(editorDir, ec);
    if (ec)
    {
        error = "Failed to create Unity Editor dir: " + editorDir.string() + " (" + ec.message() + ")";
        return false;
    }

    const std::filesystem::path scriptPath = editorDir / "SSR_SifExportImporter.cs";
    ...
    if (std::filesystem::exists(scriptPath, ec))
    {
        ...
        // Only overwrite if it's a generated script.
        if (existing.find("Generated by CppSLib") == std::string::npos)
            shouldWrite = false;
    }
    ...
    const char* script = R"(// Generated by CppSLib - SIF export importer
#if UNITY_EDITOR
using System;
...
public static class SSR_SifExportImporter
{
    [Serializable]
    private class ExportIndex
    {
        ...
        public CollisionEntry[] collisions;
    ...
    [MenuItem(\"Tools/SSR/Import Exported SIF Folder...\")]
    public static void Import()
    {
        var folderAbs = EditorUtility.OpenFolderPanel(\"Select <SIF>_SIF export folder\", Application.dataPath, \"\");
        if (string.IsNullOrEmpty(folderAbs))
            return;

        var folderRel = FileUtil.GetProjectRelativePath(folderAbs);
        if (string.IsNullOrEmpty(folderRel))
        {
            EditorUtility.DisplayDialog(\"SSR\", \"Folder must be inside this Unity project.\", \"OK\");
            return;
        }
        folderRel = folderRel.Replace('\\\\', '/');

        var exportJsonPath = folderRel + \"/export.json\";
        var exportJsonAbs = Path.GetFullPath(exportJsonPath);
        if (!File.Exists(exportJsonAbs))
        {
            EditorUtility.DisplayDialog(\"SSR\", \"export.json not found in selected folder.\", \"OK\");
            return;
        }

        var index = JsonUtility.FromJson<ExportIndex>(File.ReadAllText(exportJsonAbs));
        if (index == null)
        {
            EditorUtility.DisplayDialog(\"SSR\", \"Failed to parse export.json.\", \"OK\");
            return;
        }

        var root = new GameObject(Path.GetFileName(folderRel));
        Undo.RegisterCreatedObjectUndo(root, \"Import SIF Export\");

        TryInstantiateCollisions(folderRel, index, root.transform);
        TryCreateWaypoints(folderRel, index, root.transform);

        Selection.activeGameObject = root;
    }

    private static void TryInstantiateCollisions(string exportRootRel, ExportIndex index, Transform parent)
    {
        if (index != null && index.collisions != null && index.collisions.Length > 0)
        {
            var root = new GameObject("Collisions");
            root.transform.SetParent(parent, false);
            foreach (var entry in index.collisions)
            {
                if (entry == null || string.IsNullOrEmpty(entry.path))
                    continue;
                var assetPath = (exportRootRel + "/" + entry.path).Replace("//", "/").Replace('\\', '/');
                var collisionPrefab = AssetDatabase.LoadAssetAtPath<GameObject>(assetPath);
                if (collisionPrefab == null)
                    continue;

                var go = (GameObject)PrefabUtility.InstantiatePrefab(collisionPrefab);
                go.name = string.IsNullOrEmpty(entry.name) ? "Collision" : entry.name;
        ...
        TryInstantiateCollision(exportRootRel, index, parent);
    }

    private static void TryInstantiateCollision(string exportRootRel, ExportIndex index, Transform parent)
    {
        if (string.IsNullOrEmpty(index.collision))
            return;
        var collisionAssetPath = (exportRootRel + \"/\" + index.collision).Replace(\"//\", \"/\").Replace('\\\\', '/');
        var collisionPrefab = AssetDatabase.LoadAssetAtPath<GameObject>(collisionAssetPath);
        if (collisionPrefab == null)
            return;
    ...
    private static void TryCreateWaypoints(string exportRootRel, ExportIndex index, Transform parent)
    {
        if (string.IsNullOrEmpty(index.waypoints))
            return;

        var wpAssetPath = (exportRootRel + \"/\" + index.waypoints).Replace(\"//\", \"/\").Replace('\\\\', '/');
        var wpAbs = Path.GetFullPath(wpAssetPath);
        if (!File.Exists(wpAbs))
            return;

        var data = JsonUtility.FromJson<WaypointsFile>(File.ReadAllText(wpAbs));
        ...
        {
            if (wp == null || wp.pos == null || wp.pos.Length < 3)
                continue;

            var go = GameObject.CreatePrimitive(PrimitiveType.Cube);
            go.name = string.IsNullOrEmpty(wp.name) ? (\"WP_\" + wp.id) : wp.name;
            go.transform.SetParent(wpRoot.transform, false);
            go.transform.position = new Vector3(wp.pos[0], wp.pos[1], wp.pos[2]);
            ...
            if (mr != null)
            {
                ...
                mat.color = new Color(0.6f, 0.2f, 0.9f, 1.0f); // purple
                mr.sharedMaterial = mat;
            }
            byId[wp.id] = go.transform;
            wpById[wp.id] = wp;
        }
        ...
        linksRoot.transform.SetParent(wpRoot.transform, false);
        ...
        foreach (var link in data.links)
        {
            ...
            var go = new GameObject($\"Link_{link.from}_to_{link.to}\");
            ...
            lr.startColor = new Color(0.6f, 0.2f, 0.9f, 1.0f);
            lr.endColor = new Color(0.6f, 0.2f, 0.9f, 1.0f);

            var fromDir = GetDir(wpById, link.from, (to.position - from.position));
            var toDir = GetDir(wpById, link.to, (from.position - to.position));
            WriteHermiteSpline(lr, from.position, to.position, fromDir, toDir, Mathf.Max(link.length, Vector3.Distance(from.position, to.position)));
        }
    }

    private static Vector3 GetDir(Dictionary<int, Waypoint> wpById, int id, Vector3 fallback)
    {
        if (wpById.TryGetValue(id, out var wp) && wp != null && wp.dir != null && wp.dir.Length >= 3)
        {
            var d = new Vector3(wp.dir[0], wp.dir[1], wp.dir[2]);
    ...
    // Simple Hermite spline between waypoints for debug visualization.
    private static void WriteHermiteSpline(LineRenderer lr, Vector3 p0, Vector3 p1, Vector3 d0, Vector3 d1, float approxLen)
    {
        var t0 = d0.normalized * (approxLen * 0.33f);
        var t1 = d1.normalized * (approxLen * 0.33f);
        const int segments = 16;
        lr.positionCount = segments + 1;
        for (int i = 0; i <= segments; i++)
        {
            float t = i / (float)segments;
            lr.SetPosition(i, Hermite(p0, p1, t0, t1, t));
        }
    }
    ...
    {
        error = "Failed to write Unity Editor script: " + scriptPath.string();
        return false;
    }
    return true;
}
...
std::string HexString(std::uint32_t value, int width)
{
    std::ostringstream out;
...
bool WriteCollisionObj(const std::filesystem::path& path,
                       std::string_view objectName,
                       std::span<const Vector3> vertices,
                       std::span<const CollisionTriangle> triangles)
{
    std::ofstream obj(path);
    if (!obj)
        return false;
    ...
    auto mapIndex = [&](int idx) -> int {
        if (idx < 0 || static_cast<std::size_t>(idx) >= vertices.size())
            return -1;
        int& slot = remap[static_cast<std::size_t>(idx)];
        if (slot == -1)
        {
            slot = static_cast<int>(outVerts.size());
            outVerts.push_back(vertices[static_cast<std::size_t>(idx)]);
        }
        return slot;
    };
    ...
    {
        mapIndex(tri.Indices[0]);
        mapIndex(tri.Indices[1]);
        mapIndex(tri.Indices[2]);
    }
    ...
    for (auto const& tri : triangles)
    {
        int a = mapIndex(tri.Indices[0]);
        int b = mapIndex(tri.Indices[1]);
        int c = mapIndex(tri.Indices[2]);
        if (a < 0 || b < 0 || c < 0)
            continue;
        obj << "f " << (a + 1) << ' ' << (b + 1) << ' ' << (c + 1) << '\n';
    }
                             ...
                             std::string& error)
{
    ...
    for (std::size_t i = 0; i < numTriangles; ++i)
    {
        ...
        std::uint16_t flags = read16(off + 6);
        std::uint32_t surface = read32(off + 8);
        if (v0 >= vertices.size() || v1 >= vertices.size() || v2 >= vertices.size())
            continue;
        CollisionTriangle tri{};
        tri.Indices = {static_cast<int>(v0), static_cast<int>(v1), static_cast<int>(v2)};
        tri.Flags = flags;
        tri.SurfaceType = surface;
                             ...
                             std::filesystem::path const& unityProjectRoot)
{
    ...
    const std::filesystem::path collisionTypesDir = exportRoot / "collision_types";
    const std::filesystem::path subbranchRoot = exportRoot / "SuBranches";
    const std::filesystem::path rawRoot = exportRoot / "Raw";
    const std::filesystem::path navRoot = exportRoot / "Navigation";
    const std::filesystem::path waypointsPath = navRoot / "waypoints.json";
    const std::filesystem::path rawColiPath = rawRoot / "COLI.raw";
    const std::filesystem::path rawTrakPath = rawRoot / "TRAK.raw";
    ...
    std::filesystem::create_directories(rawRoot, ec);
    std::filesystem::create_directories(navRoot, ec);
    if (ec)
    {
...

    {
        ...
        if (!EnsureUnitySifImportEditorScript(unityRoot, scriptError))
        {
    ...
    std::string navError;
    bool hasNavigation = SeEditor::LoadNavigationFromSifChunks(parsed->Chunks, navigation, navProbe, navError);

    // Dump raw chunks for future repacking/edit workflows.
    {
        auto dumpRaw = [&](std::uint32_t type, std::filesystem::path const& outPath) {
            auto it = std::find_if(parsed->Chunks.begin(), parsed->Chunks.end(),
                                   [&](SeEditor::SifChunkInfo const& c) { return c.TypeValue == type; });
            if (it != parsed->Chunks.end())
            {
                if (!WriteBinaryFile(outPath, std::span<const std::uint8_t>(it->RawChunk.data(), it->RawChunk.size())))
                {
                    result.Error = "Failed to write raw chunk: " + outPath.string();
                    return false;
                }
                return true;
            }
            // Still create the file so tooling can rely on stable paths.
            std::ofstream empty(outPath, std::ios::binary | std::ios::trunc);
            return true;
        };

        if (!dumpRaw(0x494C4F43, rawColiPath)) // 'COLI'
            return result;
        if (!dumpRaw(0x4B415254, rawTrakPath)) // 'TRAK'
            return result;
    }

    // Export waypoints as a Unity-friendly JSON for editor tooling.
    {
        std::ofstream wpOut(waypointsPath, std::ios::binary | std::ios::trunc);
        if (!wpOut)
        {
            result.Error = "Failed to write waypoints: " + waypointsPath.string();
            return result;
        }
        ...
        {
            std::unordered_map<const SlLib::SumoTool::Siff::NavData::NavWaypoint*, int> waypointIndex;
            waypointIndex.reserve(navigation.Waypoints.size());
            for (std::size_t i = 0; i < navigation.Waypoints.size(); ++i)
            {
                auto const& wp = navigation.Waypoints[i];
                if (wp)
                    waypointIndex[wp.get()] = static_cast<int>(i);
            }

            wpOut << "{\n";
            wpOut << "  \"version\": " << navigation.Version << ",\n";
            wpOut << "  \"waypoints\": [\n";
            bool firstWp = true;
            for (std::size_t i = 0; i < navigation.Waypoints.size(); ++i)
            {
                ...
                wpOut << "    {";
                wpOut << "\"id\": " << i;
                wpOut << ", \"name\": \"" << JsonEscape(wp->Name) << "\"";
                wpOut << ", \"pos\": [" << wp->Pos.X << ", " << wp->Pos.Y << ", " << wp->Pos.Z << "]";
                wpOut << ", \"dir\": [" << wp->Dir.X << ", " << wp->Dir.Y << ", " << wp->Dir.Z << "]";
                wpOut << ", \"up\": [" << wp->Up.X << ", " << wp->Up.Y << ", " << wp->Up.Z << "]";
                wpOut << ", \"trackDist\": " << wp->TrackDist;
                wpOut << ", \"flags\": " << wp->Flags;
                wpOut << ", \"targetSpeed\": " << wp->TargetSpeed;
                wpOut << ", \"permissions\": " << wp->Permissions;
                wpOut << "}";
            }
            wpOut << "\n  ],\n";
            ...
            wpOut << "  \"links\": [\n";
            bool firstLink = true;
            for (auto const& link : navigation.Links)
            {
                if (!link || !link->From || !link->To)
                    continue;
                auto fIt = waypointIndex.find(link->From);
                auto tIt = waypointIndex.find(link->To);
                if (fIt == waypointIndex.end() || tIt == waypointIndex.end())
                    continue;
                ...
                firstLink = false;
                wpOut << "    {";
                wpOut << "\"from\": " << fIt->second;
                wpOut << ", \"to\": " << tIt->second;
                wpOut << ", \"width\": " << link->Width;
                wpOut << ", \"length\": " << link->Length;
                wpOut << "}";
            }
            wpOut << "\n  ]\n";
            wpOut << "}\n";
    ...
    std::vector<CollisionExportEntry> collisionExports;
    ...
    // Export a single collision mesh for the whole SIF from COLI chunk (Unity-readable OBJ).
    {
        ...
        if (it != parsed->Chunks.end())
        {
            ...
            {
                if (!WriteCollisionObj(collisionPath, "collision", vertices, triangles))
                {
                ...
                {
                    std::size_t operator()(CollisionKey const& k) const noexcept
                    {
                        return (static_cast<std::size_t>(k.Surface) << 16) ^ k.Flags;
                    }
                };
                struct CollisionKeyEq
                {
                    bool operator()(CollisionKey const& a, CollisionKey const& b) const noexcept
                    {
                        return a.Surface == b.Surface && a.Flags == b.Flags;
                    }
                };

                std::unordered_map<CollisionKey, std::vector<CollisionTriangle>, CollisionKeyHash, CollisionKeyEq> byType;
                byType.reserve(triangles.size());
                for (auto const& tri : triangles)
                {
                    CollisionKey key{tri.SurfaceType, tri.Flags};
                    byType[key].push_back(tri);
                }

                std::error_code dirEc;
                std::filesystem::create_directories(collisionTypesDir, dirEc);
                if (!dirEc)
                {
                    for (auto const& [key, tris] : byType)
                    {
                        const std::string surfaceHex = HexString(key.Surface, 8);
                        const std::string flagsHex = HexString(key.Flags, 4);
                        const std::string name = "collision_surface_" + surfaceHex + "_flags_" + flagsHex;
                        const std::filesystem::path outPath = collisionTypesDir / (name + ".obj");

                        if (WriteCollisionObj(outPath, name, vertices, tris))
                        {
                            WriteUnityModelMeta(outPath, StableGuidForPath(outPath));
                            collisionExports.push_back({name, outPath, key.Surface, key.Flags});
                        }
                    }
                }
            }
    ...
    json << "  \"collisions\": [\n";
    for (std::size_t i = 0; i < collisionExports.size(); ++i)
    {
        auto const& entry = collisionExports[i];
        if (i > 0)
            json << ",\n";
        json << "    {\"name\": \"" << JsonEscape(entry.Name)
             << "\", \"path\": \"" << JsonEscape(std::filesystem::relative(entry.Path, exportRoot).generic_string())
             << "\", \"surface\": " << entry.Surface
             << ", \"flags\": " << entry.Flags << "}";
    }
    json << "\n  ],\n";
    json << "  \"waypoints\": \"" << JsonEscape(std::filesystem::relative(waypointsPath, exportRoot).generic_string()) << "\",\n";
    json << "  \"raw\": {\n";
    json << "    \"COLI\": \"" << JsonEscape(std::filesystem::relative(rawColiPath, exportRoot).generic_string()) << "\",\n";
    json << "    \"TRAK\": \"" << JsonEscape(std::filesystem::relative(rawTrakPath, exportRoot).generic_string()) << "\"\n";
    json << "  },\n";
    json << "  \"forests\": [\n";
