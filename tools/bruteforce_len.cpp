...
int main(int argc, char** argv)
{
    if (argc < 3)
    {
        std::cerr << "Usage: bruteforce_len <unknown_folder> <length> [threads]\n";
        return 1;
    }

    fs::path unknownDir = argv[1];
    int L = std::max(1, std::stoi(argv[2]));
    ...
    const std::string alphabet = "_abcdefghijklmnopqrstuvwxyz";
    ...
    uint64_t total = 1;
    for (int i = 0; i < L; ++i)
        total *= static_cast<uint64_t>(alphabet.size());
    ...
    auto worker = [&]() {
        std::string name(L, 'a');
        std::string base;
        base.reserve(prefix.size() + static_cast<std::size_t>(L) + 64);
        for (;;)
        {
    ...
    std::cerr << "Elapsed: " << elapsed.count() << " sec\n";
    return 0;
}
